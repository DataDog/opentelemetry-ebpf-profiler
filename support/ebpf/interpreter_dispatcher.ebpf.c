...
static EBPF_INLINE void maybe_add_thread_context_info(Trace *trace)
{
  ...
  // TLS_READ_ABSENT means the thread has no block for the module yet, which is
  // ordinary right after a dlopen and not a failure.
  void *thread_context_buf_ptr;
  TLSReadResult rc = tls_read_var(&proc->tls, (void *)tsd_base, &thread_context_buf_ptr);
  if (rc == TLS_READ_ERR) {
    // A failure on the DTV arm also counts metricID_UnwindErrBadDTVRead, which
    // tls_read_var owns: one names the mechanism, this one the consumer.
  ...
  // values written earlier in unwind_stop. ApmCorrelationBuf carries a separate
  // trace_present flag for the same purpose, which ThreadContextBuf lacks.
  if (
...
static EBPF_INLINE int unwind_stop(struct pt_regs *ctx)
{
  ...
  // Go labels and the thread context share the union, so they are mutually
  // exclusive, Go taking precedence by running first. A thread context's trace
  // and span IDs override the apmint and OTel values written above.
