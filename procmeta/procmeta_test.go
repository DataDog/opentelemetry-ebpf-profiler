...
func TestMergeResources(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			merged := MergeResources(test.contributions)
			if test.expectNil {
...
func TestMergeResourcesDoesNotModifyContributions(t *testing.T) {
	...
	merged := MergeResources([]*pcommon.Resource{first, second})
	require.NotNil(t, merged)
...
func TestMergeResourcesSharesSingleContribution(t *testing.T) {
	...
	require.Same(t, only, MergeResources([]*pcommon.Resource{nil, only, nil}))
}
