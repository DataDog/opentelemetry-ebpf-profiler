...
// (User Statically-Defined Tracepoint) probes for heap profiling.
//
// Probes are discovered by scanning the `.note.stapsdt` sections of the
// executable mappings of each tracked process, filtered to the heap
...
// in libdd-heap-sampler and is expected to track the eventual OTel-standard
// heap-profiling provider name once that is defined.
