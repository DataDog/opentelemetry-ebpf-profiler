...
void init_thread_context(size_t attrs_data_size) {
  custom_labels_current_set_v2 =
      malloc(sizeof(custom_labels_v2_tl_record_t) + attrs_data_size);
...
void update_thread_context(uint64_t span_id, uint64_t trace_id_lo,
  ...
  memcpy(custom_labels_current_set_v2->trace_id, &trace_id_lo,
         sizeof(trace_id_lo));
  memcpy(custom_labels_current_set_v2->trace_id + sizeof(trace_id_lo),
  ...
  for (size_t i = 0; i < attrs_count; ++i) {
    ...
    custom_labels_current_set_v2->attrs_data[attr_pos++] = value_length;
    memcpy(custom_labels_current_set_v2->attrs_data + attr_pos, attrs[i].value,
           value_length);
