...
const (
	...
	// tlsExportSize is the size of the pointer that thread-local holds.
	tlsExportSize = 8
)
...
type threadcontextData struct {
	// tlsVar is how tlsExport is accessed, classified once per file.
	tlsVar tls.Var
}
...
func (d *threadcontextData) Attach(ebpf interpreter.EbpfHandler, pid libpf.PID,
	...
	var procInfo support.ThreadContextProcInfo
	var pendingTLS *tls.VarLocation
	if tlsInfo, err := loc.VarInfo(libc.DTVInfo{}); err == nil {
	...
	return &Instance{procInfo: &procInfo, pendingTLS: pendingTLS}, nil
}
...
func (i *Instance) UpdateLibcInfo(ebpf interpreter.EbpfHandler, pid libpf.PID,
	...
	if err = ebpf.UpdateProcData(libpf.ThreadContext, pid, unsafe.Pointer(i.procInfo)); err != nil {
		return err
	}
	i.pendingTLS = nil
