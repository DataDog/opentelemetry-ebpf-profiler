...
type Tracker struct {
	...
	// liveHeapPIDs is the set of PIDs whose binary has the ddheap:free
	// probe. Only these PIDs get tracked — without the free probe, allocs
...
// (i.e., has the ddheap:free probe attached). Only PIDs with support
...
// No-op if the PID hasn't been marked as supporting live heap.
...
// DroppedAllocs returns and resets the count of allocations dropped due to
// the global cap being reached.
func (t *Tracker) DroppedAllocs() uint64 {
	t.mu.Lock()
	defer t.mu.Unlock()
	d := t.dropped
	t.dropped = 0
